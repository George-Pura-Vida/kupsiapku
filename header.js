...
function loadCart(){if(window.KupSiApkuCart)return Promise.resolve(window.KupSiApkuCart);return new Promise((resolve,reject)=>{let s=document.querySelector('script[data-kupsiapku-cart]');if(s){if(window.KupSiApkuCart)return resolve(window.KupSiApkuCart);s.addEventListener('load',()=>resolve(window.KupSiApkuCart),{once:true});s.addEventListener('error',reject,{once:true});return}s=document.createElement('script');s.src='/cart.js?v=20260926-5';s.dataset.kupsiapkuCart='1';s.onload=()=>resolve(window.KupSiApkuCart);s.onerror=reject;document.head.append(s)})}
/* Make legacy onclick="add('...')" buttons work even before cart.js finishes loading. */
window.add=window.add||function(id){loadCart().then(c=>c&&c.add(id)).catch(()=>{})};
if(product){const hero=document.querySelector('.hero,.productHero');const actions=hero?.querySelector('.actions');if(actions){const keep=[...actions.querySelectorAll('a[href^="#"]')];actions.innerHTML=`<button class="btn primary" type="button" data-cart-add="${product}">🛒 ${en?'Add to cart':'Přidat do košíku'}</button><button class="btn secondary" type="button" data-cart-buy="${product}">⚡ ${en?'Order':'Objednat'}</button>`;keep.forEach(a=>actions.append(a))}}
loadCart().then(c=>c&&c.render()).catch(()=>console.error('KupSiApku: cart.js se nepodařilo načíst'));
})();
