...
async function loadPayouts(){const token=getToken();if(!token)return;try{const data=await apiPayout('summary',{headers:{'X-Affiliate-Token':token}});const s=data.summary;document.querySelector('#payoutAvailable').textContent=(s.availableMinor/100).toLocaleString('cs-CZ')+' Kč';const button=document.querySelector('#requestPayout');button.disabled=s.availableMinor<s.minimumMinor;document.querySelector('#payoutHint').textContent=button.disabled?'Výplata je dostupná od 1 000 Kč.':'Můžete požádat o výplatu celé dostupné částky.';}catch(error){setStatus(error.message,'error');}}
async function apiPayout(action,options={}){const response=await fetch('/api/payout.php?action='+action,{...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});const data=await response.json().catch(()=>({ok:false,error:'Server vrátil neplatnou odpověď.'}));if(!response.ok)throw new Error(data.error||'Operaci se nepodařilo dokončit.');return data;}
async function loadProfile(){
  const token=getToken(); if(!token)return;
  try{const data=await api('profile',{headers:{'X-Affiliate-Token':token}});render({...data.profile,billing:data.billing},data.stats);loadPayouts();setStatus('Profil a statistiky jsou načtené ze serveru.','success');}
...
document.querySelector('#requestPayout').addEventListener('click',async()=>{const token=getToken();if(!token)return;setStatus('Vytvářím žádost a fakturu…');try{const data=await apiPayout('request',{method:'POST',headers:{'X-Affiliate-Token':token},body:'{}'});setStatus(data.message,'success');loadPayouts();}catch(error){setStatus(error.message,'error');}});
