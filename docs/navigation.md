...
- There is one navigation element on each page. At 1100px and below it becomes a collapsible menu. Wider layouts keep branding, links and languages on one row.
...
With Playwright and Chromium installed, run `node tests/header_test.cjs` (set `NODE_PATH` if Playwright is installed outside this repository). It checks every HTML page at 320, 375, 768, 800, 801, 1024, 1100, 1101, 1194, 1250, 1251, 1366 and 1920px; navigation destinations and fragments; overlap; Enter/Space/Tab/Escape; ARIA state; cart updates; language section mapping; and the no-JavaScript fallback.
